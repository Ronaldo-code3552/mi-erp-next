...
export function useCrud<T>(service: any, empresaId: string, initialFilters: any = {}) {
  ...
  const fetchData = useCallback(async (page = 1, term = searchTerm, activeFilters = filters) => {
    setLoading(true);
    try {
      const response: ApiResponse<T[]> = await service.getByEmpresa(empresaId, page, 20, term, activeFilters);
