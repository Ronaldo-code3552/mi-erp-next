...
export default function DocumentoComercialPage() {
    ...
    const handleAnular = async (row: DocumentoCompra) => {
        ...
        if (!id || estadoOf(row).toUpperCase().includes("ANUL")) return;
    ...
    const columns = [
        ...
        { header: "Estado", width: "110px", className: "text-center", render: (row: DocumentoCompra) => <span className={`inline-flex rounded border px-2 py-1 text-[10px] font-bold ${badgeClass(estadoOf(row))}`}>{estadoOf(row)}</span> },
        {
            ...
            render: (row: DocumentoCompra) => {
                const id = documentId(row);
                const isAnulado = estadoOf(row).toUpperCase().includes("ANUL");
                return (
                    <div className="flex items-center justify-center gap-1">
                        ...
                        <button type="button" disabled={isAnulado} onClick={() => id && router.push(`/dashboard/documento-comercial/editar/${id}`)} className="rounded p-1.5 text-slate-400 hover:bg-blue-50 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-35" title="Editar"><IconEdit size={17} /></button>
                        <button type="button" disabled={isAnulado} onClick={() => handleAnular(row)} className="rounded p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600 disabled:cursor-not-allowed disabled:opacity-35" title="Anular"><IconBan size={17} /></button>
                    </div>
                );
            }
        }
