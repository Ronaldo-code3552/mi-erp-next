...
export default function VerNotaSalidaPage() {
    const router = useRouter();
    const params = useParams<{ id: string }>();
    const notaId = String(params?.id || '');

    const [loading, setLoading] = useState(true);
    const [nota, setNota] = useState<NotaSalidaResponse | null>(null);

    useEffect(() => {
        const load = async () => {
            if (!notaId) {
                toast.error('Nota de salida inválida.');
            ...
            setLoading(true);
            try {
                const res = await notaSalidaService.getById(notaId);
                if (!res?.isSuccess || !res?.data) {
                    toast.error(res?.message || 'No se pudo cargar la nota de salida.');
                    router.push('/dashboard/notas-salida');
                    return;
                }
                setNota(res.data);
    ...
    const transaccionRules = useMemo(() => {
        ...
        return {
            esTrasladoInterno: ['TE', 'SA', 'YG', 'DO', 'ZX', 'SM', 'SP', 'YC', 'YF', 'YR', 'YV'].includes(t),
            allowImportDocs: ['SV', 'SB', 'CG', 'SU', 'TE', 'YG', 'DD'].includes(t),
            esCambioCodigo: t === 'SC',
    ...
    const documentDescription =
        nota?.tipoDocumentoComercial?.descripcion ||
        nota?.tipodoccomercialDesc ||
        ...
        <div className="p-6 max-w-[95%] mx-auto pb-20 animate-fade-in-up">
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800">Detalle Nota de Salida</h1>
                        <p className="text-xs text-slate-500">Visualización de documento {nota.notassalidaId}</p>
                    </div>
                </div>
                ...
                <div className="col-span-12 lg:col-span-8 space-y-6">
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <ReadonlySelect label="Tipo Transacción" value={transactionDescription} />
                            ...
                            {transaccionRules.allowImportDocs && (
                                <>
                                    <ReadonlySelect label="Tipo Documento" value={documentDescription} />
                                    <FormInput label="N° Documento Ref." value={documentReferenceValue} />
                                </>
                            )}
                        </div>
                    </div>
                                        ...
                                        nota.detalles.map((item: NotaSalidaDetalle, idx: number) => (
                                            <tr key={`${item.bienId}-${idx}`} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="p-3">
                                                    ...
                                                    {(transaccionRules.esCambioCodigo || transaccionRules.esCambioPresentacion) && (
